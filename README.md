...
*   **自定义等级制换算**：可指定哪些课程采用等级制，并为每个等级设置对应的百分制与 GPA；设置会同步影响课程 GPA、配色、平均分、学期/累计 GPA 和图表统计。
...
### v7.1 - 自定义等级制换算
*   新增“等级换算设置”按钮，可按课程名称启用自定义等级制。
*   支持按 `等级 = 百分制, GPA` 配置换算关系，并保存在本地浏览器中。
...
*   **模拟成绩**：
    ...
    3.  观察 GPA 和图表的实时变化。
    4.  再次点击按钮退出模拟，恢复真实成绩。
*   **自定义等级制课程**：点击 `等级换算设置`，在第一栏逐行填写课程名称（需与成绩单显示完全一致），在第二栏逐行填写换算关系，例如 `A = 95, 4.0`。保存后立即生效。
