...
*   **自定义等级制换算**：无需指定课程名；为 A+ 至 F 逐项设置 GPA 或百分制后，所有等级制课程都会统一使用该规则。设置会同步影响课程 GPA、配色、平均分、学期/累计 GPA 和图表统计。
...
### v7.1 - 自定义等级制换算
*   新增“等级换算设置”按钮，为所有等级制课程启用统一转换规则。
*   提供逐等级数字输入框，并可选择“直接映射 GPA”或“转换为百分制”。
...
*   **模拟成绩**：
    ...
    3.  观察 GPA 和图表的实时变化。
    4.  再次点击按钮退出模拟，恢复真实成绩。
*   **自定义等级制课程**：点击 `等级换算设置`，选择“直接映射 GPA”或“转换为百分制”，然后逐项填写 A+ 至 F 的数值。保存后，所有等级制课程立即按该规则重新计算。
